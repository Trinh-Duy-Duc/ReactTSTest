...
const loginApi = async (body: LoginBody) => {
    const resp = await repoAxiosInternalMethod._post<LoginResponse>('/auth/login', body);
    return resp;
}
const userInfoApi = async () =>{
    const resp = await repoAxiosInternalMethod._get<UserInfoRespone>('/auth/userinfo');
    return resp;
}
